			       ...
			       bfd_size_type uncompressed_size)
{
  ...
  if (compressed)
      ...
      if (orig_compression_header_size == 0)
	{
	  ...
	  uncompressed_size
	    = get_uncompressed_size (uncompressed_buffer + 4);
	}
      else
	{
  ...
  if (compressed)
    {
      ...
      bfd_update_compression_header (abfd, compressed_buffer, sec);
      memmove (compressed_buffer + compression_header_size,
  ...
  else
    {
      ...
      if (compress ((Bytef*) compressed_buffer + header_size,
		    &compressed_size,
      ...
      if (compressed_size < uncompressed_size)
	{
	  ...
	  /* Write the zlib header.  In this case, it should be "ZLIB"
	     ...
	     big-endian order.  */
	  memcpy (compressed_buffer + compression_header_size, "ZLIB", 4);
	  compressed_buffer[header_size - 1] = size; size >>= 8;
	  compressed_buffer[header_size - 2] = size; size >>= 8;
	  compressed_buffer[header_size - 3] = size; size >>= 8;
	  compressed_buffer[header_size - 4] = size; size >>= 8;
	  compressed_buffer[header_size - 5] = size; size >>= 8;
	  compressed_buffer[header_size - 6] = size; size >>= 8;
	  compressed_buffer[header_size - 7] = size; size >>= 8;
	  compressed_buffer[header_size - 8] = size;
	}
      else
	{
				       ...
				       int *compression_header_size_p)
{
  ...
  if (compressed)
	...
	{
	  bfd_size_type uncompressed_size
	    = get_uncompressed_size ((bfd_byte *) header
...
bfd_init_section_decompress_status (bfd *abfd, sec_ptr sec)
{
  ...
  uncompressed_size = get_uncompressed_size (header + header_size - 8);
  if (compression_header_size != 0
