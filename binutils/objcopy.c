	   ...
	   const bfd_arch_info_type *input_arch)
{
  ...
  switch (do_debug_sections)
	...
	{
	  if (ibfd->xvec->flavour != bfd_target_elf_flavour)
	    fatal (_("--compress-debug-sections=zlib-gabi is unsupported for `%s'"),
		   bfd_get_target (ibfd));
	...
	case OPTION_COMPRESS_DEBUG_SECTIONS:
	  if (optarg)
		...
		do_debug_sections = decompress;
	      else if (strncmp (optarg, "zlib", strlen (optarg)) == 0)
		do_debug_sections = compress_zlib;
	      else if (strncmp (optarg, "zlib-gnu", strlen (optarg)) == 0)
		do_debug_sections = compress_gnu_zlib;
	      else if (strncmp (optarg, "zlib-gabi", strlen (optarg)) == 0)
